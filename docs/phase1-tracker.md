...
- [x] **C5** Regressions in `metronome_verify`: grid-follow tracks a ramping grid; re-anchor mid-run glitch-free; `start_at` frame-exact. JSI note: `uint64_t` frames cross as JS `double` — no BigInt. 3 tests added (213→218 checks), verification-only, no production change; drifting re-anchor isolated to `SeekGridCursor` (only new test fails), start_at frame-exact via `FirstNonzeroFrame` bypassing the ±2 detector window. Both reviewers pass.
