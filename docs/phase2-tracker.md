...
**In progress:** none.
**Next:** Wave 3 — `P2-A4` (#31) native build integration (Gradle externalNativeBuild + iOS podspec), now unblocked (#29, #30 landed). This is where the TurboModule + HostObject first compile/link against the built core; the C++ scaffolding under `packages/core-native/cpp/` is uncompiled until then. Leads toward the Wave 5 device gate (#33, §13.3), which the loop cannot self-clear.
