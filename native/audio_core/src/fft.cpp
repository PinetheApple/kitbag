  ...
  for (int len = 2; len <= n; len *= 2) {
    ...
    for (int i = 0; i < n; i += len) {
      ...
      for (int j = 0; j < len / 2; ++j) {
        const float tr = wr * data[2 * (i + j + len / 2)] -
                         wi * data[2 * (i + j + len / 2) + 1];
        const float ti = wr * data[2 * (i + j + len / 2) + 1] +
                         wi * data[2 * (i + j + len / 2)];
        data[2 * (i + j + len / 2)] = data[2 * (i + j)] - tr;
        data[2 * (i + j + len / 2) + 1] = data[2 * (i + j) + 1] - ti;
        data[2 * (i + j)] += tr;
        data[2 * (i + j) + 1] += ti;
